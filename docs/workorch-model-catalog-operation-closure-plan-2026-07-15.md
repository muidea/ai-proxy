...
2. `config.Load`：operations 必填、case-fold 唯一、容量完整、唯一 RouteOwner。
...
10. `api_error.go`、`handler_test_helpers.go`、本文档已纳入 Git 索引。
11. `go test ./...`、`go vet ./...`、`gofmt -l` 通过；本地 `config.yaml` readiness 通过。
...
- operation 使用规范化稳定顺序。
- model 按 case-fold ID 后稳定排序。
- GET、POST `/v1/models` 返回相同业务合同。
- `owned_by` 只返回确定的 enabled provider 名称。
- response 不包含 provider secret、BaseURL 或内部 fallback 配置。
...
配置 map key 与 `ModelInfo.ID` 规范化后冲突时必须启动失败。禁止允许 `GPT-4o` 和 `gpt-4o` 同时存在，
避免 WorkOrch 刷新整个目录时因 case-fold duplicate 拒绝。
...
- 唯一匹配：将该 provider 作为确定 `owned_by`。

禁止继续使用 `owned_by=ai-proxy` 掩盖零匹配或多匹配。`owned_by` 只是输出字段，但其值必须来自已经通过
启动校验的确定路由结果。
...
3. 输出 operations、容量和确定 owned_by。
...
8. 若保留公开 `NewHandler(Config, ...)`，其 fail-fast 检查必须覆盖已知 operation/capability、容量关系、
   case-fold 唯一、RouteOwner 存在且匹配 model、operation readiness、provider protocol 合法性，以及
...
4. README 删除 `owned_by` 不确定时回退 `ai-proxy` 的旧说明。
...
- case-fold duplicate model ID 拒绝。
...
- `NewHandler` 对畸形 operation/capability、容量关系、RouteOwner 匹配和 case-fold duplicate fail-fast，或构造器
...
2. `TestRequireResolvedConfigMalformedTable`：覆盖 unknown protocol、unknown/duplicate/unsorted capability、
   unknown/duplicate/unsorted operation、非法容量、case-fold duplicate、错误 RouteOwner 和 operation readiness。
...
2. 确认每个 model 返回 operations、容量和确定 owned_by。
3. 使用 chat model 调用 `/v1/chat/completions` 成功。
4. 使用 embedding model 调用 `/v1/embeddings` 成功。
